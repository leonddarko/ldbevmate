...
import { BadgeCheck, BadgeX, CheckCircle2, CircleX, DoorOpen, Dot, EllipsisVerticalIcon, EvCharger, HousePlug, IdCardLanyard, LayoutDashboardIcon, Map, MapIcon, MoveRight, Shield, ShieldCheck, ShieldOff } from "lucide-react";
                ...
                <div
                    className="
                      bg-blue-50/80
                        ...
                        "
                >
                    <div className="flex justify-between md:justify-start items-center gap-4 px-2 py-4 md:py-6 border-b border-zinc-200 ">
                        <Link href="/cpo/dashboard">
                            <div className="flex justify-start items-center gap-2">
                                ...
                                <div className="w-full md:w-32">
                                    ...
                                    {cpo?.verified === true ?
                                        <BadgeCheck size={15} className="text-green-900 " />
                                        : <BadgeX size={15} className=" text-amber-800 " />}
                                </div>
                            </div>
                        </Link>
                        ...
                        <div className="dropdown dropdown-end md:hidden">
                            <button tabIndex={0} role="button"
                                className="btn py-0 p-2 border-0 text-blue-950 bg-transparent hover:bg-zinc-100 shadow-none">
                                <EllipsisVerticalIcon size={20} />
                            </button>
                            <ul tabIndex={0} className="dropdown-content menu rounded-3xl w-80 p-2 shadow-md bg-blue-50/90 backdrop-blur-3xl border border-white/90">
                                {session.user.role === "admin" && (<>
                                    <li>
                                        <Link href="/admin" className="flex justify-between items-center gap-2 text-black/40 transition-all">
                                            <div className="flex items-center gap-2 text-blue-950/50">
                                                <Shield size={20} className={`text-blue-950`} />
                                                <span className={`font-semibold font-notosans text-sm transition-all`}>
                                                    Admin Dashboard
                                                </span>
                                            </div>
                                            <Dot size={15} className="" />
                                        </Link>
                                    </li>
                                </>)}
                    ...
                    {/* User Id */}
                    {session && (
                        <div className="flex justify-between items-center gap-1.5 bg-blue-100/50 text-black/40 p-4 transition-all border border-zinc-100 rounded-b-3xl md:rounded-none">
                    ...
                    <ul tabIndex={0} className="dropdown-content menu z-4 w-auto ps-4 py-2 hidden md:block border border-zinc-100 ">
                        {session.user.role === "admin" && (
                            <>
                    ...
                    <button
                        ...
                        className="hidden md:flex btn-ghost justify-between items-center gap-2  text-black/40 ps-4 py-4 px-2 w-full border-t border-zinc-200 hover:bg-red-50 transition-all cursor-pointer">
                        <div className="flex items-center gap-2 text-black">
